<x-school-layout title="Programs">
    <div class="mb-6 rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
        ...
        <form method="POST" action="{{ route('school.programs.store') }}" class="mt-4 grid gap-4 sm:grid-cols-2">
            @csrf
            <div>
            ...
            <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-100" x-data="{ editing: false }">
                <div x-show="!editing" class="flex items-start justify-between gap-4">
                    ...
                    <div class="flex shrink-0 gap-2">
                        ...
                        <form method="POST" action="{{ route('school.programs.destroy', $program) }}" onsubmit="return confirm('Delete this program?')">
                            @csrf @method('DELETE')
                ...
                <form x-show="editing" method="POST" action="{{ route('school.programs.update', $program) }}" class="grid gap-3 sm:grid-cols-2">
                    @csrf @method('PUT')
